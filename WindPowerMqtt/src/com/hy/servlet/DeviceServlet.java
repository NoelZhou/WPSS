...
public class DeviceServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String sql="SELECT wd.id,  group_concat(wdif.ip order by wdif.ip separator ';   ')as ip,dev.device_type_id,wd.create_time,wd.name,wd.run_state,mbt.name as mtname FROM mqtt_device wd left join windpower_deviceinfo wdif on wd.device_id=wdif.device_id left join windpower_device dev on wd.device_id=dev.id left join windpower_devicetype  wpdt on wpdt.id=dev.device_type_id left join modbustcp_type mbt on mbt.id=wpdt.modbus_type group by wd.id";
